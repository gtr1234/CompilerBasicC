...
void postorder(Node* root)
{
    ...
    do
    {
		...
		if(root->num_children == 0){
		   ...
		   root->eval = malloc(strlen(root->value) + 1);
		   strcpy(root->eval,root->value);
		}
		else{
		   ...
		   if(cnt == 1){
		     ...
		     printf("root->eval =%s", root->eval);
				     ...
				     if (root->children[sing].eval[0] > 48 && root->children[sing].eval[0] <57 ){
				     	...
				     	printf("value =  %.2f \n", temp1);//root->children[sing].eval);
				     }
				     else{
				    
					for(i=0;i<100;i++){
						
						if( strcmp(root->children[sing].eval, table[i])){
					...
					for(i=0;i<100;i++){
						...
						if( strcmp(root->children[doub].eval, table[i])){
